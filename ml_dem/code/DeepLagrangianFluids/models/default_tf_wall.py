...
class MyParticleNetwork(tf.keras.Model):
    ...
    def calc_collision_wall(self,pos):
        """ Apply particle-wall collision, move particle away from wall
        try a cylinder
        """
        #print(pos.shape)
        if pos.shape[0] is None: #skip initilization run
            return
        ymax=1.0
        cx=0.5
        cz=0.5
        cr=0.5
        #bottom wall
        bwTensor = tf.constant([self.particle_radius,self.particle_radius,self.particle_radius])
        pos = tf.maximum(pos,bwTensor)
        #top wall
        twTensor = tf.constant([ymax,ymax,ymax])
        pos = tf.minimum(pos,twTensor)
        #side wall, cylinder
        mask=np.zeros(pos.shape)
        mask[:,1] = 1
        ccTensor = pos*mask + cx*(1-mask)
        mask[:,0] = 1
        ccTensor = ccTensor*mask + cz*(1-mask)
        #print(pos)
        #print(ccTensor)
        #dist2wallTensor = (pos-ccTensor)**2
        #dist2wallTensor = tf.constant([cr]) - self.euclidean_distance(pos,ccTensor)
        dist2wallTensor = tf.constant([cr]) - self.euclidean_distance(pos,ccTensor)
        dist2wallTensor = tf.reshape(dist2wallTensor,[dist2wallTensor.shape[0],1])
        directionTensor = -(pos - ccTensor)/dist2wallTensor
        displaceMag = tf.constant([self.particle_radius])-dist2wallTensor
        displaceMag = tf.minimum(0.0,displaceMag)
        displaceTensor = directionTensor * displaceMag
        pos = pos + displaceTensor

        '''
        element-wise assignment is not supported in Tensorflow
        for i in range(pcount):            
            #top wall
            pos[i][1] = tf.minimum(pos[i][1],ymax)
            #side wall, cylinder
            dist2wall = cr-sqrt((pos[i][0]-cx)*(pos[i][0]-cx)+(pos[i][2]-cz)*(pos[i][2]-cz))
            if dist2wall <self.particle_radius:
                nx = (pos[i][0]-cx)/(cr-dist2wall)
                nz = (pos[i][2]-cz)/(cr-dist2wall)
                pos[i][0] -= nx*(self.particle_radius-dist2wall)
                pos[i][2] -= nz*(self.particle_radius-dist2wall)
        '''

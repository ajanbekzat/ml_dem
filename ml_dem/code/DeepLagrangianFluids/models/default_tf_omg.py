...
class MyParticleNetwork(tf.keras.Model):
    ...
    def __init__(
        ...
        self.layer_channels = [32, 64, 64, 6]
    ...
    def compute_new_pos_vel(self, pos1, vel1, pos2, vel2, omg2, pos_correction, omg_correction):
    ...
    def compute_correction(self,
        ...
        fluid_feats = [tf.ones_like(pos[:, 0:1]), vel, omg]
        ...
        return self.pos_correction[:,0:3], self.pos_correction[:,3:6]
    ...
    def call(self, inputs, fixed_radius_search_hash_table=None):
        ...
        """
        pos, vel, omg, feats, box, box_feats = inputs

        pos2, vel2 = self.integrate_pos_vel(pos, vel)
        
        pos_correction, omg_correction = self.compute_correction(
            pos2, vel2, omg, feats, box, box_feats, fixed_radius_search_hash_table)
        
        pos2_corrected, vel2_corrected, omg2_corrected = self.compute_new_pos_vel(
            pos, vel, pos2, vel2, omg, pos_correction, omg_correction)

        return pos2_corrected, vel2_corrected, omg2_corrected
    ...
    def init(self, feats_shape=None):
        ...
        omg = np.zeros(shape=(1, 3), dtype=np.float32)
        ori = np.zeros(shape=(1, 3), dtype=np.float32)
        ...
        _ = self.__call__((pos, vel, omg, feats, box, box_feats))

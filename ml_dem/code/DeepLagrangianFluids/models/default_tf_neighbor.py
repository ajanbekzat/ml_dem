...
class MyParticleNetwork(tf.keras.Model):
    ...
    def compute_correction(self,
        ...
        #------------------------------ need to run a conv to get count of neighbor particles
        Add_neighbor_count_to_feature=True
        if Add_neighbor_count_to_feature:
            particleNum = pos[:, 0:1].shape[0]
            ...
            self.ans_conv0_fluid = self.conv0_fluid(fluid_feats, pos, pos,
                                                    filter_extent)
            #---------------------------------------------------------------------
            # add fluid neighbors to feature and start the first layer
            ...
            #if not particleNum is None:
            #print(particleNum)
            #print(self.num_fluid_neighbors)
            if particleNum>1:
                nbs = tf.reshape(self.num_fluid_neighbors,pos[:, 0:1].shape)            
                fluid_feats = [nbs, vel]
                if not other_feats is None:
                    fluid_feats.append(other_feats)
                fluid_feats = tf.concat(fluid_feats, axis=-1)
            
            #tf.print("Actual feature shape: {:}".format(fluid_feats.shape))
        else:
        ...
        if False:
            ...
            print(self.ans_conv0_fluid.shape)
        ...
        for conv, dense in zip(self.convs, self.denses):
            ...
            if False:
                print("inp_feats, ans_conv,ans_dense, ans, lenans_convs")
                print(inp_feats.shape)
                print(ans_conv.shape)
                print(ans_dense.shape)
                print(ans.shape)
                print(len(self.ans_convs))
                
        if not Add_neighbor_count_to_feature:
            self.num_fluid_neighbors = ml3d.ops.reduce_subarrays_sum(
                tf.ones_like(self.conv0_fluid.nns.neighbors_index,
                             dtype=tf.float32),
                self.conv0_fluid.nns.neighbors_row_splits)
           ...
           # print("=============================")
            #print(self.num_fluid_neighbors)
           # import sys
           # tf.print(self.num_fluid_neighbors,output_stream=sys.stdout)

...
from evaluate_network_mew import evaluate_tf as evaluate
...
dataset_path = "/lustre/scratch/lul/tfdata/50hzdrummew_data"
train_dir = os.path.join(dataset_path, 'baseMew')
lluTimeStep = 1.0/50
...
def create_model():
    from models.default_tf_mew import MyParticleNetwork
    """Returns an instance of the network for training and evaluation"""
    model = MyParticleNetwork(particle_radius=PARTICLE_RADIUS, filter_scale=6, timestep=lluTimeStep)
...
def main():
    ...
    def train(model, batch, first_batch):
        with tf.GradientTape() as tape:
            ...
            for batch_i in range(batch_size):
                inputs = ([
                   batch['pos0'][batch_i], batch['vel0'][batch_i], batch['viscosity0'][batch_i],
                ...
                inputs = (pr_pos1, pr_vel1, batch['viscosity0'][batch_i], batch['box'][batch_i],
    ...
    while trainer.keep_training(checkpoint.step,
       ...
       # print("step={:} checkpoint={:}".format(trainer.current_step,manager.latest_checkpoint))
        batch_tf = {}
        for k in ('viscosity0','pos0', 'vel0', 'pos1', 'pos2', 'box', 'box_normals'):

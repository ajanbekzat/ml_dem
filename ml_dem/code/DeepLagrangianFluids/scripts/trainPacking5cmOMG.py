...
from datasets.dataset_reader_physics_omg import read_data_train, read_data_val
from collections import namedtuple
from glob import glob
import time
import tensorflow as tf
from utils.deeplearningutilities.tf import Trainer, MyCheckpointManager
from evaluate_network_omg import evaluate_tf as evaluate
...
dataset_path = "/lustre/scratch/lul/tfdata/50hzpackingomg_data"
train_dir = os.path.join(dataset_path, 'results_f6')
lluTimeStep =0.02
lluFilter= 6
...
def create_model():
    from models.default_tf_omg import MyParticleNetwork
    """Returns an instance of the network for training and evaluation"""
    model = MyParticleNetwork(particle_radius=PARTICLE_RADIUS, filter_scale=lluFilter, timestep=lluTimeStep)
...
def main():
    ...
    def loss_fn(pr_pos, gt_pos, pr_omg, gt_omg, num_fluid_neighbors):
        gamma = 1
        neighbor_scale = 1 / 40
        importance = 1#tf.exp(-neighbor_scale * num_fluid_neighbors)
        loss_pos = tf.reduce_mean(importance *
                              euclidean_distance(pr_pos, gt_pos)**gamma)
        
        scale = PARTICLE_RADIUS*lluTimeStep
        loss_omg = tf.reduce_mean(importance *
                              euclidean_distance(pr_omg*scale, gt_omg*scale)**gamma)
        
        return loss_pos+loss_omg
    ...
    def train(model, batch, first_batch):
        with tf.GradientTape() as tape:
            ...
            for batch_i in range(batch_size):
                inputs = ([
                    batch['pos0'][batch_i], batch['vel0'][batch_i], batch['omg0'][batch_i], None,
                    batch['box'][batch_i], batch['box_normals'][batch_i]
                ])

                pr_pos1, pr_vel1, pr_omg1 = model(inputs)

                l = 0.5 * loss_fn(pr_pos1, batch['pos1'][batch_i], pr_omg1, batch['omg1'][batch_i],
                                  model.num_fluid_neighbors)

                inputs = (pr_pos1, pr_vel1, pr_omg1, None, batch['box'][batch_i],
                          batch['box_normals'][batch_i])
                pr_pos2, pr_vel2, pr_omg2 = model(inputs)

                l += 0.5 * loss_fn(pr_pos2, batch['pos2'][batch_i],pr_omg2, batch['omg2'][batch_i],
                                   model.num_fluid_neighbors)
                losses.append(l)
    ...
    while trainer.keep_training(checkpoint.step,
       ...
       # print("step={:} checkpoint={:}".format(trainer.current_step,manager.latest_checkpoint))
        batch_tf = {}
        for k in ('pos0', 'vel0', 'pos1', 'pos2','omg0','omg1','omg2' ,'box', 'box_normals'):

...
train_dir = os.path.join(dataset_path, 'baseloss5050frames3w127batch64')
lluTimeStep =0.02
lluFilter= 6
...
train_params = TrainParams(50 * _k//hvd.size(), 0.001*hvd.size(), 64)
    ...
    def train(model, batch, first_batch):
        with tf.GradientTape() as tape:
            ...
            for batch_i in range(batch_size):
                ...
                l += 0.7 * loss_fn(pr_pos3, batch['pos3'][batch_i],
                                   model.num_fluid_neighbors)
                losses.append(l)

...
train_dir = os.path.join(dataset_path, 'results-1GPU-filterScale6-importance1-gamma1-3frames')
lluTimeStep =0.02
lluFilter= 6 
...
def create_model():
    from models.default_tf_gauss import MyParticleNetwork
...
def main():
    ...
    def train(model, batch, first_batch):
        with tf.GradientTape() as tape:
            ...
            for batch_i in range(batch_size):
                ...
                l = 0.3333 * loss_fn(pr_pos1, batch['pos1'][batch_i],
                ...
                l += 0.3333 * loss_fn(pr_pos2, batch['pos2'][batch_i],
                                   model.num_fluid_neighbors)
                ...
                l += 0.3333 * loss_fn(pr_pos3, batch['pos3'][batch_i],
                                   model.num_fluid_neighbors)
                losses.append(l)
    ...
    while trainer.keep_training(checkpoint.step,
       ...
       # print("step={:} checkpoint={:}".format(trainer.current_step,manager.latest_checkpoint))
        batch_tf = {}
        for k in ('pos0', 'vel0', 'pos1', 'pos2', 'pos3', 'box', 'box_normals'):

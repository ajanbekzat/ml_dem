...
dataset_path = "/lustre/scratch/lul/tfdata/50hzhopper5cm_data"
train_dir = os.path.join(dataset_path, 'baseloss5050frames7')
lluTimeStep =0.02
lluFilter= 6
...
def main():
    ...
    def train(model, batch, first_batch):
        with tf.GradientTape() as tape:
            ...
            for batch_i in range(batch_size):
                ...
                l = 0.1 * loss_fn(pr_pos1, batch['pos1'][batch_i],
                ...
                l += 0.1 * loss_fn(pr_pos2, batch['pos2'][batch_i],
                                   model.num_fluid_neighbors)
                
                inputs = (pr_pos2, pr_vel2, None, batch['box'][batch_i],
                          batch['box_normals'][batch_i])
                pr_pos3, pr_vel3 = model(inputs)

                l += 0.1 * loss_fn(pr_pos3, batch['pos3'][batch_i],
                                   model.num_fluid_neighbors)
                #next frame
                inputs = (pr_pos3, pr_vel3, None, batch['box'][batch_i],
                          batch['box_normals'][batch_i])
                pr_pos4, pr_vel4 = model(inputs)

                l += 0.1 * loss_fn(pr_pos4, batch['pos4'][batch_i],
                                   model.num_fluid_neighbors)
                #next frame
                inputs = (pr_pos4, pr_vel4, None, batch['box'][batch_i],
                          batch['box_normals'][batch_i])
                pr_pos5, pr_vel5 = model(inputs)
                l += 0.1 * loss_fn(pr_pos5, batch['pos5'][batch_i],
                                   model.num_fluid_neighbors)
                #next frame
                inputs = (pr_pos5, pr_vel5, None, batch['box'][batch_i],
                          batch['box_normals'][batch_i])
                pr_pos6, pr_vel6 = model(inputs)
                l += 0.1 * loss_fn(pr_pos6, batch['pos6'][batch_i],
                                   model.num_fluid_neighbors)
                #next frame
                inputs = (pr_pos6, pr_vel6, None, batch['box'][batch_i],
                          batch['box_normals'][batch_i])
                pr_pos7, pr_vel7 = model(inputs)
                l += 0.4 * loss_fn(pr_pos7, batch['pos7'][batch_i],
                                   model.num_fluid_neighbors)
                losses.append(l)
    ...
    while trainer.keep_training(checkpoint.step,
       ...
       # print("step={:} checkpoint={:}".format(trainer.current_step,manager.latest_checkpoint))
        batch_tf = {}
        for k in ('pos0', 'vel0', 'pos1', 'pos2', 'pos3','pos4','pos5','pos6','pos7', 'box', 'box_normals'):

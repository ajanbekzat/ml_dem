        ...
        if not os.path.isfile(outfilepath):            
            ...
            for frame_i in sublist:
                ...
                for flid in fluid_ids:
                    ...
                    pos_, vel_, omg_,ori_, density_ = mfixomg_numpy_from_bgeo(bgeo_path)
                    pos.append(pos_)
                    vel.append(vel_)
                    omg.append(omg_)
                    ori.append(ori_)
                ...
                feat_dict['omg'] = omg.astype(np.float32)
                feat_dict['ori'] = ori.astype(np.float32)

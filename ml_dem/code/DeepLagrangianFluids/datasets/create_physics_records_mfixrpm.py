...
def stl_to_particles(rpm, objpath, radius=None):
    if radius is None:
...
def create_scene_files(rpm, scene_dir, scene_id, outfileprefix, splits=1):

#    with open(os.path.join(scene_dir, 'scene.json'), 'r') as f:
#        scene_dict = json.load(f)
    box, box_normals = stl_to_particles(rpm, os.path.join(scene_dir, 'geometry.stl'))
    ...
    fluid_id_bgeo_map = {
        k: mfix_get_fluid_bgeo_files(partio_dir, k) for k in fluid_ids
    }
        ...
        if not os.path.isfile(outfilepath):            
            ...
            for frame_i in sublist:
                ...
                if frame_i==0:
                    ...
                    if pos_.shape[:][0] != np0:
                        validCase = False
                        print("------------ERROR-----frameID={:d}, np={:d}, np0={:d}".format(frame_i,pos_.shape[:][0],np0))
                        break
...
def main():
    ...
    for scene_dir in scene_dirs:
        rpm = 10 + scene_id//11*10
        scene_id +=1
        print(scene_dir, scene_id, rpm)
        scene_name = os.path.basename(scene_dir)
        print(scene_name)
        outfileprefix = os.path.join(outdir, scene_name)
        create_scene_files(rpm, scene_dir, scene_name, outfileprefix, args.splits)

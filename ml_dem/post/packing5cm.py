...
def readVTK(fname):
    ...
    for j in range(1):
        ...
        lx=avgx-x0
        aor = avgy *2#asin(lx/dist)*180/pi
...
ke=[]
time=[]
rootdir = "/lustre/scratch/lul/tfdata/TestingCases/packing_000010/"
dirNameList  = ["PARTIO/"]
...
#    fileName = rootdir+dirNameList[i]+"/tfResults.txt"
    ...
    for j in range(ldata):
        ...
        lx=apx[j]-0.5
        iiaor = apy[j]*2#asin(lx/dist)*180/pi
        saor.append(iiaor)
...
plt.xlabel('Time (s)')
plt.ylabel('Packing height (m)')
plt.ylim(0,40)
plt.savefig(rootdir+'packingBase{:d}.png'.format(rid))
plt.show()
plt.clf()
...
plt.savefig(rootdir+'packingBaseGT{:d}.png'.format(rid))
plt.show()
plt.clf()
...
for i in range(ncase):
    fileName = rootdir+"baseLoss5050_{:d}/tfResults.txt".format(i*5000+50000)
#    fileName = rootdir+dirNameList[i]+"/tfResults.txt"
    data=np.loadtxt(fileName,skiprows=0, usecols = (0,1,2,3,4,5,6))
    simTime=data[:,0]*0.02
    apx = data[:,1]
...
plt.ylabel('Packing height (m)')
plt.ylim(0,40)
plt.savefig(rootdir+'packingBaseLoss5050{:d}.png'.format(rid))
plt.show()
plt.clf()
...
plt.ylabel('Granular temperature ($m^2$/$s^2$)')
axe.legend(ncol=3,loc='upper right', prop={'size': 5})
plt.savefig(rootdir+'packingBaseLoss5050GT{:d}.png'.format(rid))
plt.show()
plt.clf()

#
label_string=["Micro-scale", "Multi-scale (50-50)"]
fig,axe=plt.subplots(1,1)  
data=[]
data.append(baseAOR)
...
plt.savefig(rootdir+'packingheightDOE{:d}.png'.format(rid))
plt.show()
plt.clf()
############################
ndata=len(baseSteps)

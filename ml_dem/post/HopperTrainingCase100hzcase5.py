...
ke=[]
time=[]
rootdir = "/lustre/scratch/lul/tfdata/TestingCases/100hzhopper5cmcase5/"
dirNameList  = ["PARTIO/"]
...
for i in range(ncase):
    ...
    for j in range(ldata):
        iiaor = apy[j]
        saor.append(iiaor)        
    plt.plot(simTime,saor,'-.',label="0.02s,Steps=({:d}k)".format(nsteps//1000))  
########################## 100Hz ###############
rootdir = "/lustre/scratch/lul/tfdata/TestingCases/100hzhopper5cmcase5/"
for i in range(ncase):
    nsteps=i*1000+case0
    fileName = rootdir+"100hzbaseloss5050frames50_{:d}/tfResults.txt".format(nsteps)
    data=np.loadtxt(fileName,skiprows=0, usecols = (0,1,2,3,4,5,6))
    simTime=data[:,0]*0.01
    apy = data[:,2]    
    saor=[]   
    ...
    for j in range(ldata):
        iiaor = apy[j]
        saor.append(iiaor)        
    plt.plot(simTime,saor,'-',label="0.01s Steps=({:d}k)".format(nsteps//1000))    
#####################################################

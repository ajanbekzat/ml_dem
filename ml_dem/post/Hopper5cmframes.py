...
plt.plot(time,aor,'-o',color='black',label="MFiX-DEM")
########################## ADD SIM DATA ############
...
#    fileName = rootdir+dirNameList[i]+"/tfResults.txt"
    ...
    plt.plot(simTime,saor,'-.',label="Conv-DEM (F=2)")
...
for i in range(ncase):
    ...
    for j in range(ldata):
        iiaor = apy[j]
        saor.append(iiaor)        
    plt.plot(simTime,saor,'--',label="Conv-DEM (F=3)")    
#####################################################
#########################################
for i in range(ncase):
    nsteps=i*1000+10000
    ...
    for j in range(ldata):
        iiaor = apy[j]
        saor.append(iiaor)        
    #plt.plot(simTime,saor,'-',label="N+3")    
#####################################################
...
for i in range(ncase):
    ...
    for j in range(ldata):
        iiaor = apy[j]
        saor.append(iiaor)        
    plt.plot(simTime,saor,'-.',label="Conv-DEM (F=5)")    
#####################################################
#########################################
for i in range(ncase):
    nsteps=i*1000+10000
    fileName = rootdir+"baseloss5050frames7_{:d}/tfResults.txt".format(nsteps)
    data=np.loadtxt(fileName,skiprows=0, usecols = (0,1,2,3,4,5,6))
    simTime=data[:,0]*0.02
    apy = data[:,2]    
    saor=[]   
    ...
    for j in range(ldata):
        iiaor = apy[j]
        saor.append(iiaor)        
    plt.plot(simTime,saor,'--',label="Conv-DEM (F=7)")   
    #########################################
for i in range(ncase):
    nsteps=i*1000+10000
    fileName = rootdir+"baseloss5050frames7w1_{:d}/tfResults.txt".format(nsteps)
    data=np.loadtxt(fileName,skiprows=0, usecols = (0,1,2,3,4,5,6))
    simTime=data[:,0]*0.02
    apy = data[:,2]    
    saor=[]   
    ...
    for j in range(ldata):
        iiaor = apy[j]
        saor.append(iiaor)        
    #plt.plot(simTime,saor,'-',label="N+7, w1")  
#####################################################

...
for i in range(ncase):
    fileName = rootdir+"baseLose_{:d}/tfResults.txt".format(i*5000+50000)
#    fileName = rootdir+dirNameList[i]+"/tfResults.txt"
    data=np.loadtxt(fileName,skiprows=0, usecols = (0,1,2,3,4,5,6))
    simTime=data[:,0]*0.02
    apx = data[:,1]
    ...
    plt.plot(simTime,saor,label="Steps"+"-{:d}k ({:.1f})".format(i*5+50,avgAOR))
...
for i in range(ncase):
    nsteps=i*5000+50000
    plt.plot(simTime,baseLoseKE[i],label="Steps"+"-{:d}k ({:.2f})".format(nsteps//1000,baseLoseAVGKE[i]))
plt.xlabel('Time (s)')
plt.ylabel('Granular temperature ($m^2$/$s^2$)')
axe.legend(ncol=3,loc='upper right', prop={'size': 5})
plt.savefig(rootdir+'drumKE_base_steps_{:d}.png'.format(rid))
...
plt.plot(time,aor,'-.',color='black',label="MFiX ({:.1f})".format(avgExpAOR))
for i in range(ncase):
    fileName = rootdir+"baseLose8020_{:d}/tfResults.txt".format(i*5000+50000)
#    fileName = rootdir+dirNameList[i]+"/tfResults.txt"
    data=np.loadtxt(fileName,skiprows=0, usecols = (0,1,2,3,4,5,6))
    simTime=data[:,0]*0.02
    apx = data[:,1]
    ...
    avgke = sum(ake[-AVS:])/AVS    
    baseLose2AVGKE.append(avgke)
    baseLose2KE.append(ake)
    avgAOR = sum(saor[-AVS:])/AVS
    baseLose2AOR.append(avgAOR)
    plt.plot(simTime,saor,label="L2Steps"+"-{:d}k ({:.1f})".format(i*5+50,avgAOR))
    

axe.legend(ncol=2,loc='lower right', prop={'size': 6})
plt.xlabel('Time (s)')
plt.ylabel('AoR (degree)')
plt.ylim(0,40)
plt.savefig(rootdir+'drumAoR_baseLose2_{:d}.png'.format(rid))
plt.show()
plt.clf()
...
for i in range(ncase):
    nsteps=i*5000+50000
    plt.plot(simTime,baseLose2KE[i],label="L2Steps"+"-{:d}k ({:.2f})".format(nsteps//1000,baseLoseAVGKE[i]))
plt.xlabel('Time (s)')
plt.ylabel('Granular temperature ($m^2$/$s^2$)')
axe.legend(ncol=3,loc='upper right', prop={'size': 5})
plt.savefig(rootdir+'drumKE_baseLose2_steps_{:d}.png'.format(rid))
plt.show()
plt.clf()
...
plt.plot(time,aor,'-.',color='black',label="MFiX ({:.1f})".format(avgExpAOR))
for i in range(ncase):
    fileName = rootdir+"baseLose3_{:d}/tfResults.txt".format(i*5000+50000)
#    fileName = rootdir+dirNameList[i]+"/tfResults.txt"
    data=np.loadtxt(fileName,skiprows=0, usecols = (0,1,2,3,4,5,6))
    simTime=data[:,0]*0.02
    apx = data[:,1]
    ...
    avgke = sum(ake[-AVS:])/AVS    
    baseLose3AVGKE.append(avgke)
    baseLose3KE.append(ake)
    avgAOR = sum(saor[-AVS:])/AVS
    baseLose3AOR.append(avgAOR)
...
for i in range(ncase):
    nsteps=i*5000+50000
    plt.plot(simTime,baseLose3KE[i],label="LSteps"+"-{:d}k ({:.2f})".format(nsteps//1000,baseLose3AVGKE[i]))
plt.xlabel('Time (s)')
plt.ylabel('Granular temperature ($m^2$/$s^2$)')
axe.legend(ncol=3,loc='upper right', prop={'size': 5})
plt.savefig(rootdir+'drumKE_base_steps_{:d}.png'.format(rid))
...
############
##################################
label_string=["Micro Loss", "Multi(80-20)","Multi(50-50)", "Multi(20-80)"]
fig,axe=plt.subplots(1,1)  
data=[]
data.append(baseAOR)

data.append(baseLose2AOR)
data.append(baseLoseAOR)
data.append(baseLose3AOR)
...
plt.plot(baseSteps,baseLose2AOR,'-p',color='red', label=label_string[1])
plt.plot(baseSteps,baseLoseAOR,'-s',color='blue', label=label_string[2])
plt.plot(baseSteps,baseLose3AOR,'-*',color='green', label=label_string[3])

axe.legend(ncol=1, prop={'size': 5})
plt.xlabel('Model at training steps (-)')
plt.ylabel('AoR (degree)')
...
data.append(baseLose2AVGKE)
data.append(baseLoseAVGKE)
data.append(baseLose3AVGKE)
...
plt.plot(baseSteps,baseLose2AVGKE,'-p',color='red', label=label_string[1])
plt.plot(baseSteps,baseLoseAVGKE,'-s',color='blue', label=label_string[2])
plt.plot(baseSteps,baseLose3AVGKE,'-*',color='green', label=label_string[3])
axe.legend(ncol=1, prop={'size': 5})
plt.xlabel('Model at training steps (-)')
plt.ylabel('Granular temperature ($m^2$/$s^2$)')
...
print(sum(baseLose2AOR)/ndata)
print(sum(baseLoseAOR)/ndata)
print(sum(baseLose3AOR)/ndata)
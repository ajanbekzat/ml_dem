...
#plt.plot(time,aor,'-o',color='black',label="MFiX-DEM (h=0.4m)")
########################## ADD SIM DATA ############
...
#    fileName = rootdir+dirNameList[i]+"/tfResults.txt"
    ...
    plt.plot(simTime,err,'-',color='red',label="h=0.3m")
...
#    fileName = rootdir+dirNameList[i]+"/tfResults.txt"
    ...
    plt.plot(simTime,err,'-.',color='green',label="h=0.4m")
...
#    fileName = rootdir+dirNameList[i]+"/tfResults.txt"
    ...
    avgAOR = sum(saor[-AVS:])/AVS
    baseLoseAOR.append(avgAOR)
    plt.plot(simTime,err,'--',color='blue',label="h=0.5m")
...
plt.xlabel('Time (s)')
plt.ylabel('H(Conv-DEM)/H(MFiX-DEM)-1')
#plt.ylim(0.22,0.32)
#plt.xlim(0,4)

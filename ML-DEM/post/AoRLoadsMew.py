...
for i in range(ncase):
    nsteps=i*5000+50000
    fileName = rootdir+"baseMew_{:d}/tfResults.txt".format(nsteps)
...
for i in range(ncase):
    fileName = rootdir+"baseLoseMew_{:d}/tfResults.txt".format(i*5000+50000)
#    fileName = rootdir+dirNameList[i]+"/tfResults.txt"
    data=np.loadtxt(fileName,skiprows=0, usecols = (0,1,2,3,4,5,6))
    simTime=data[:,0]*0.02
    apx = data[:,1]
...
#
label_string=["Micro-loss", "Multi-scale loss (50-50)"]
fig,axe=plt.subplots(1,1)  
data=[]
data.append(baseAOR)

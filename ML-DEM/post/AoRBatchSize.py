...
for i in range(ncase):
    fileName = rootdir+"fitbaseLoseBatch32_{:d}/tfResults.txt".format(i*1000+5000)
#    fileName = rootdir+dirNameList[i]+"/tfResults.txt"
    data=np.loadtxt(fileName,skiprows=0, usecols = (0,1,2,3,4,5,6))
    simTime=data[:,0]*0.02
    apx = data[:,1]
...
plt.plot(time,aor,'-.',color='black',label="MFiX ({:.1f})".format(avgExpAOR))
for i in range(ncase):
    fileName = rootdir+"fitbaseLoseBatch64_{:d}/tfResults.txt".format(i*1000+5000)
#    fileName = rootdir+dirNameList[i]+"/tfResults.txt"
    data=np.loadtxt(fileName,skiprows=0, usecols = (0,1,2,3,4,5,6))
    simTime=data[:,0]*0.02
    apx = data[:,1]
    ...
    plt.plot(simTime,saor,label="L2Steps"+"-{:d}k ({:.1f})".format(i*1+5,avgAOR))
...
############
##################################
label_string=["Batch-16", "Batch-32","Batch-64"]
fig,axe=plt.subplots(1,1)  
data=[]
data.append(baseAOR)
...
plt.ylabel('AoR (degree)')
plt.savefig(rootdir+'base_baselosebatch_aor{:d}.png'.format(rid))
plt.show()
plt.clf()
############################
ndata=len(baseSteps)
...
plt.plot(baseSteps,baseLoseAOR,'-s',color='red', label=label_string[1])
plt.plot(baseSteps,baseLose2AOR,'-p',color='blue', label=label_string[2])
...
plt.xlabel('Epochs (-)')
plt.ylabel('AoR (degree)')
plt.savefig(rootdir+'drumAoR_base_batchsteps_{:d}.png'.format(rid))
plt.show()
plt.clf()
...
plt.xlabel('Loss function')
plt.ylabel('Granular temperature ($m^2$/$s^2$)')
plt.savefig(rootdir+'base_baselose_batchgt{:d}.png'.format(rid))
plt.show()
plt.clf()
############################
fig,axe=plt.subplots(1,1)
plt.plot(baseSteps,baseAVGKE,'-o',color='black', label=label_string[0])
plt.plot(baseSteps,baseLoseAVGKE,'-s',color='red', label=label_string[1])
plt.plot(baseSteps,baseLose2AVGKE,'-p',color='blue', label=label_string[2])
#plt.plot(baseSteps,baseLose3AVGKE,'-*',color='green', label=label_string[3])
axe.legend(ncol=1, prop={'size': 5})
plt.xlabel('Epochs (-)')
plt.ylabel('Granular temperature ($m^2$/$s^2$)')
plt.savefig(rootdir+'drumAoR_base_stepsbatch_gt_{:d}.png'.format(rid))
plt.show()
plt.clf()
############
ndata=len(baseAOR)
